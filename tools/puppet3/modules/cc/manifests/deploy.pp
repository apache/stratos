...
define cc::deploy ($service, $security, $target, $owner, $group) {
  file { "/tmp/${cc::deployment_code}":
    ensure       => present,
    owner        => $owner,
    group        => $group,
    sourceselect => all,
  ...
  exec {
    "Copy_${name}_modules_to_carbon_home":
      ...
      command => "cp -r /tmp/${cc::deployment_code}/* ${target}/; chown -R ${owner}:${owner} ${target}/; chmod -R 755 ${target}/",
      require => File["/tmp/${cc::deployment_code}"];

    "Remove_${name}_temporory_modules_directory":
      path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/java/bin/',
      command => "rm -rf /tmp/${cc::deployment_code}",
