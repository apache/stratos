...
curl -X POST -H "Content-Type: application/json" -d "@${network_partitions_path}/network-partition-2.json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/networkPartitions
...
curl -X POST -H "Content-Type: application/json" -d "@${cartridges_path}/tomcat.json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/cartridges

echo "Adding esb cartridge..."
curl -X POST -H "Content-Type: application/json" -d "@${cartridges_path}/esb.json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/cartridges

echo "Adding php cartridge..."
curl -X POST -H "Content-Type: application/json" -d "@${cartridges_path}/php.json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/cartridges

echo "Adding esb-php-group group..."
curl -X POST -H "Content-Type: application/json" -d "@${cartridges_groups_path}/esb-php-group.json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/cartridgeGroups
...
curl -X POST -H "Content-Type: application/json" -k -v -u ${username}:${password} https://${host_ip}:${host_port}/api/applications/cartridge-group-app/deploy/application-policy-1
