...
public class MockIPAddressPool {
    ...
    private MockIPAddressPool() {
        try {
            ...
            privateIpSequence = (AtomicInteger) persistenceManager.read(MOCK_IAAS_PRIVATE_IP_SEQUENCE);
        ...
        try {
            publicIpSequence = (AtomicInteger) persistenceManager.read(MOCK_IAAS_PUBLIC_IP_SEQUENCE);
    ...
    public String getNextPrivateIpAddress() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Mock private IP address allocated: " + ipAddress);
        }
        return ipAddress;
    }
    ...
    public String getNextPublicIpAddress() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Mock public IP address allocated: " + ipAddress);
        }
        return ipAddress;
    }
    ...
    private void persistInRegistry(String resourcePath, Serializable serializable) {
        try {
            persistenceManager.persist(resourcePath, serializable);
        } catch (RegistryException e) {
