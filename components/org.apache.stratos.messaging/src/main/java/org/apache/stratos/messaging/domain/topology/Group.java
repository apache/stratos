...
public class Group  implements Subscribable,Composite, Serializable {
	...
	public String findKillBehavior(String alias) {
		...
		List<Group> all_groups = getAllGroups();
		...
		for (Group group : all_groups) {
			kill_behavior = group.getKillBehaviorForAlias(alias);
			if (Dependencies.KILL_UNDEFINED.equals(kill_behavior)) {
				...
				return group.getKillBehaviorForAlias(alias);
			}
		}
	...
	public List<Group> getAllGroups () {
		...
		while (keyit.hasNext()) {
			...
			if (!(obj instanceof Group)) {
				...
				if (log.isDebugEnabled()) {
					log.debug("adding nested groups / composite to result list " + group.getAlias());
			    }
				List<Group> sub_results = group.getAllGroups();
	...
	public Group findGroup(String alias) {
		List<Group> all_groups = getAllGroups();
		for (Group group : all_groups) {
			if (log.isDebugEnabled()) {
				log.debug("findGroup in group  " + group.getAlias() + " for alias " + alias);
			}
			if (group.subscribables.containsKey(alias)) {
				return group;
			} else {
	...
	public Map<String, String> getDownStreamDependenciesAsMap (String alias) {
		...
		for (int i = 0; i < results.size(); i++ ) {
			...
			Group gr = s.getHomeGroup(); // TODO - need to change return type to Group
	...
	public Map<String, String> getUpstreamDependenciesAsMap (String alias) {
		...
		for (int i = 0; i < results.size(); i++) {
			Subscribable s = results.get(i);
			if (s != null) {
				Group home_gr = s.getHomeGroup(); // TODO - need to change return type to Group
				String key = null;
				String val = null;
				if (home_gr != null) {
					key =  s.getAlias();
	...
	public List<Subscribable>  getUpstreamDependencies(Subscribable subscribable) {
		...
		if (subscribable instanceof Scalable)  {
			...
			if (parent == null) {
				...
				Group parentGroup = (Group) parentSubscribable;
				if (log.isDebugEnabled()) {
					log.debug("continue upstream search in parent group " + parentGroup.alias + " for " + alias  );
				}
				// continue with group alias as dependency alias (dependent would be group)
				List<Subscribable> list = parentGroup.getUpstreamDependencies(this);
				if (log.isDebugEnabled()) {
					log.debug("found " + list.size() + " items in upstream search in parent group " + parentGroup.alias + " for " + alias  );
				}
				upstream.addAll(list);
			}
		} else if (subscribable instanceof Group) {
			...
			if (parent == null) {
				...
				// continue with group alias as dependency alias (dependent would be group)
				List<Subscribable> list = ((Group) parent).getUpstreamDependencies(this);
				if (log.isDebugEnabled()) {
