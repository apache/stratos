...
public class TopologyBuilder {
    ...
    public static void handleServiceRemoved(List<Cartridge> cartridgeList) {
        ...
        for (Cartridge cartridge : cartridgeList) {
            ...
            if (service.getClusters().size() == 0) {
                ...
                log.warn("Subscription already exists. Hence not removing the service:" +
                         cartridge.getType() + " from the topology");
            }
        }
    }
    ...
    public static void handleClusterCreated(ClusterStatusClusterCreatedEvent event) {
        ...
        try {
            ...
            if (service.clusterExists(event.getClusterId())) {
                ...
                cluster = new Cluster(event.getServiceName(), event.getClusterId(),
                                      event.getDeploymentPolicyName(),
        ...
        for (Cluster cluster : appClusters) {
            ...
            for (PortMapping portMapping : cartridge.getPortMappings()) {
                ClusterPortMapping clusterPortMapping =
                        new ClusterPortMapping(appId, cluster.getClusterId(), portMapping.getName(),
    ...
    public static void handleClusterRemoved(ClusterContext ctxt) {
        ...
        if (!service.clusterExists(ctxt.getClusterId())) {
            log.warn(String.format("Cluster %s does not exist for service %s", ctxt.getClusterId(),
    ...
    public static void handleMemberCreatedEvent(MemberContext memberContext) {
        ...
        String autoscalingReason =
                memberContext.getProperties().getProperty(StratosConstants.SCALING_REASON)
                             .getValue();
        long scalingTime = Long.parseLong(
                memberContext.getProperties().getProperty(StratosConstants.SCALING_TIME).getValue());
        ...
        try {
            TopologyManager.acquireWriteLock();
            Member member =
                    new Member(service.getServiceName(), clusterId, memberId, clusterInstanceId,
                               networkPartitionId, partitionId,
                               memberContext.getLoadBalancingIPType(), initTime);
            member.setStatus(MemberStatus.Created);
            member.setLbClusterId(lbClusterId);
            member.setProperties(
                    CloudControllerUtil.toJavaUtilProperties(memberContext.getProperties()));
            cluster.addMember(member);
            TopologyManager.updateTopology(topology);
            Long timeStamp = System.currentTimeMillis();
            //publishing to BAM
            BAMUsageDataPublisher
                    .publish(memberContext.getMemberId(), memberContext.getPartition().getId(),
                             ...
                             MemberStatus.Created.toString(), timeStamp, autoscalingReason,
                             scalingTime, null);
        } finally {
            TopologyManager.releaseWriteLock();
        }
    ...
    private static int findKubernetesServicePort(String clusterId,
        ...
        throw new RuntimeException(
                "Kubernetes service port not found: [cluster-id] " + clusterId + " [port] " +
                portMapping.getPort());
    }
            ...
            try {
                ...
                if (!member.isStateTransitionValid(MemberStatus.Starting)) {
                    ...
                    BAMUsageDataPublisher.publish(instanceStartedEvent.getMemberId(),
                                                  ...
                                                  instanceStartedEvent.getServiceName(),
                                                  MemberStatus.Starting.toString(), timeStamp, null,
                                                  null, null);
                }
            } finally {
                TopologyManager.releaseWriteLock();
    ...
    public static void handleMemberActivated(InstanceActivatedEvent instanceActivatedEvent) {
        ...
        MemberActivatedEvent memberActivatedEvent =
                new MemberActivatedEvent(instanceActivatedEvent.getServiceName(),
                                         instanceActivatedEvent.getClusterId(),
        ...
        try {
            ...
            if (!member.isStateTransitionValid(MemberStatus.Active)) {
                ...
                try {
                    ...
                    for (PortMapping portMapping : portMappings) {
                        if (kubernetesServices != null) {
                            portValue = findKubernetesServicePort(clusterId, kubernetesServices,
                                                                  portMapping);
                        } else {
                ...
                BAMUsageDataPublisher.publish(memberActivatedEvent.getMemberId(),
                                              ...
                                              memberActivatedEvent.getServiceName(),
                                              MemberStatus.Active.toString(), timeStamp, null, null,
                                              null);
            }
        } finally {
            TopologyManager.releaseWriteLock();
    ...
    public static void handleMemberReadyToShutdown(
        ...
        MemberReadyToShutdownEvent memberReadyToShutdownEvent =
                new MemberReadyToShutdownEvent(instanceReadyToShutdownEvent.getServiceName(),
                                               instanceReadyToShutdownEvent.getClusterId(),
        ...
        BAMUsageDataPublisher.publish(instanceReadyToShutdownEvent.getMemberId(),
                                      ...
                                      instanceReadyToShutdownEvent.getClusterInstanceId(),
                                      instanceReadyToShutdownEvent.getServiceName(),
                                      MemberStatus.ReadyToShutDown.toString(), timeStamp, null,
                                      null, null);
    ...
    public static void handleMemberMaintenance(
        ...
        MemberMaintenanceModeEvent memberMaintenanceModeEvent =
                new MemberMaintenanceModeEvent(instanceMaintenanceModeEvent.getServiceName(),
                                               instanceMaintenanceModeEvent.getClusterId(),
    ...
    public static void handleMemberTerminated(String serviceName, String clusterId,
        ...
        TopologyEventPublisher
                .sendMemberTerminatedEvent(serviceName, clusterId, memberId, clusterInstanceId,
                                           networkPartitionId, partitionId, properties, groupAlias);
    }
    ...
    public static void handleClusterActivatedEvent(
        ...
        ClusterInstanceActivatedEvent clusterInstanceActivatedEvent =
                new ClusterInstanceActivatedEvent(clusterStatusClusterActivatedEvent.getAppId(),
    ...
    public static void handleClusterInactivateEvent(
        ...
        ClusterInstanceInactivateEvent clusterInactivatedEvent1 =
                new ClusterInstanceInactivateEvent(clusterInactivateEvent.getAppId(),
                                                   clusterInactivateEvent.getServiceName(),
                                                   clusterInactivateEvent.getClusterId(),
        ...
        try {
            ...
            if (context.isStateTransitionValid(status)) {
                ...
                log.error(String.format("Cluster state transition is not valid: [cluster-id] %s " +
                                        " [instance-id] %s [current-status] %s [status-requested] %s",
                                        clusterInactivateEvent.getClusterId(),
                                        clusterInactivateEvent.getInstanceId(), context.getStatus(),
                                        status));
                return;
            }
        } finally {
    ...
    private static void deleteAppResourcesFromMetadataService(
        ...
        try {
            ...
            log.error(
                    "Error occurred while deleting the application resources frm metadata service ",
                    e);
        }
    }
        ...
        try {
            ...
            if (context.isStateTransitionValid(status)) {
                ...
                ClusterInstanceTerminatedEvent clusterTerminatedEvent =
                        new ClusterInstanceTerminatedEvent(event.getAppId(), event.getServiceName(),
                                                           event.getClusterId(),
                                                           event.getInstanceId());
    ...
    public static void handleClusterTerminatingEvent(ClusterStatusClusterTerminatingEvent event) {
        ...
        try {
            ...
            if (!cluster.isStateTransitionValid(ClusterStatus.Terminating, event.getInstanceId())) {
                log.error(
                        "Invalid state transfer from " + cluster.getStatus(event.getInstanceId()) +
                        " to " +
