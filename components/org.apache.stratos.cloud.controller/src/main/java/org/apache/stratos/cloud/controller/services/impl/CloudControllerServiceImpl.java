...
public class CloudControllerServiceImpl implements CloudControllerService {
	...
	@Override public ServiceGroup getServiceGroup(String name) throws InvalidServiceGroupException {
	...
	@Override public MemberContext[] startInstances(InstanceContext[] instanceContexts)
	...
	public MemberContext startInstance(InstanceContext instanceContext)
		...
		try {
			...
			IaasProvider iaasProvider =
					CloudControllerContext.getInstance().getIaasProviderOfPartition(cartridge.getType(), partitionId);
	...
	private MemberContext createMemberContext(String applicationId, String cartridgeType, String memberId,
		...
		MemberContext memberContext =
				new MemberContext(applicationId, cartridgeType, instanceContext.getClusterId(), memberId);
	...
	@Override public boolean terminateInstances(String clusterId) throws InvalidClusterException {
	...
	@Override public boolean registerService(Registrant registrant) throws CartridgeNotFoundException {
	...
	@Override public Cartridge getCartridge(String cartridgeType) throws CartridgeNotFoundException {
		Cartridge cartridge = CloudControllerContext.getInstance().getCartridge(cartridgeType);
		if (cartridge != null) {
			return cartridge;
		}
	...
	@Override public boolean unregisterService(String clusterId) throws UnregisteredClusterException {
		...
		if (cartridge == null) {
			String msg = String.format(
					"Service unregistration failed. No matching cartridge found: [cartridge-type] %s " +
		...
		Runnable unregister = new Runnable() {
			public void run() {
				...
				try {
					...
					if (ctxt == null) {
						String msg = String.format(
								"Service unregistration failed. Cluster not found: [cluster-id] %s " +
								"[application-id] %s ", clusterId_, ctxt.getApplicationId());
						log.error(msg);
						return;
					}
	 ...
	 */
	@Override public boolean validateDeploymentPolicyNetworkPartition(String cartridgeType, String networkPartitionId)
		...
		try {
			...
			for (Partition partition : networkPartition.getPartitions()) {
				if (validatedPartitions != null && validatedPartitions.contains(partition.getId())) {
					...
					IaasProvider iaasProvider =
							CloudControllerContext.getInstance().getIaasProvider(cartridge.getType(), provider);
			...
			for (Entry<String, Future<IaasProvider>> entry : jobList.entrySet()) {
				...
				try {
					...
					String message =
							"Could not cache partitions against the cartridge: [cartridge-type] " + cartridgeType;
					log.error(message, e);
	...
	@Override public boolean updateClusterStatus(String serviceName, String clusterId, String instanceId,
	...
	@Override public boolean createApplicationClusters(String appId, ApplicationClusterContext[] appClustersContexts)
		...
		try {
							...
							for (ClusterPortMapping clusterPortMapping : clusterPortMappings) {
								try {
									if (clusterPortMapping.isKubernetes()) {
										URL accessUrl =
												new URL(clusterPortMapping.getProtocol(), appClusterCtxt.getHostName(),
			...
			for (ApplicationClusterContext appClusterCtxt : appClustersContexts) {
				ClusterContext clusterContext =
						new ClusterContext(appId, appClusterCtxt.getCartridgeType(), appClusterCtxt.getClusterId(),
	...
	@Override public KubernetesCluster getKubernetesCluster(String kubernetesClusterId)
	...
	@Override public KubernetesMaster getMasterForKubernetesCluster(String kubernetesClusterId)
			throws NonExistingKubernetesClusterException {
		return CloudControllerContext.getInstance().getKubernetesMasterInGroup(kubernetesClusterId);
	}

	@Override public KubernetesHost[] getHostsForKubernetesCluster(String kubernetesClusterId)
			throws NonExistingKubernetesClusterException {
		return CloudControllerContext.getInstance().getKubernetesHostsInGroup(kubernetesClusterId);
	}

	@Override public boolean addKubernetesCluster(KubernetesCluster kubernetesCluster)
	...
	@Override public boolean updateKubernetesCluster(KubernetesCluster kubernetesCluster)
	...
	@Override public boolean addKubernetesHost(String kubernetesClusterId, KubernetesHost kubernetesHost)
		...
		try {
			...
			if (kubernetesCluster.getKubernetesHosts() == null) {
				...
				kubernetesHostArrayList =
						new ArrayList<KubernetesHost>(Arrays.asList(kubernetesCluster.getKubernetesHosts()));
			}
	...
	@Override public boolean removeKubernetesCluster(String kubernetesClusterId)
			throws NonExistingKubernetesClusterException {
		if (StringUtils.isEmpty(kubernetesClusterId)) {
	...
	@Override public boolean removeKubernetesHost(String kubernetesHostId) throws NonExistingKubernetesHostException {
	...
	@Override public boolean updateKubernetesMaster(KubernetesMaster kubernetesMaster)
	...
	@Override public boolean updateKubernetesHost(KubernetesHost kubernetesHost)
	...
	@Override public boolean addNetworkPartition(NetworkPartition networkPartition)
	...
	@Override public boolean removeNetworkPartition(String networkPartitionId)
	...
	@Override public boolean updateNetworkPartition(NetworkPartition networkPartition)
			throws NetworkPartitionNotExistsException {
		try {
			handleNullObject(networkPartition, "Network Partition is null");
			handleNullObject(networkPartition.getId(), "Network Partition ID is null");
	...
	@Override public NetworkPartition getNetworkPartition(String networkPartitionId) {
		try {
			return CloudControllerContext.getInstance().getNetworkPartition(networkPartitionId);
		} catch (Exception e) {
			String message =
					String.format("Could not get network partition: [network-partition-id] %s", networkPartitionId);
			log.error(message);
			throw new CloudControllerException(message, e);
		}
	}
