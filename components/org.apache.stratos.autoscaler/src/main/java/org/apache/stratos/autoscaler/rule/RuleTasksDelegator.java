...
public class RuleTasksDelegator {
	...
	public double getPredictedValueForNextMinute(float average, float gradient,
	                                             float secondDerivative, int timeInterval) {
		double predictedValue;
		//        s = u * t + 0.5 * a * t * t
		if (log.isDebugEnabled()) {
			log.debug(String.format(
					"Predicting the value, [average]: %s , [gradient]: %s , [second derivative] " +
					": %s , [time intervals]: %s ", average, gradient, secondDerivative,
					timeInterval));
		}
		predictedValue = average + gradient * timeInterval +
		                 0.5 * secondDerivative * timeInterval * timeInterval;
	...
	public int getNumberOfInstancesRequiredBasedOnMemoryConsumption(float threshold,
	                                                                double predictedValue, int min,
	                                                                int max) {
	...
	public int getNumberOfInstancesRequiredBasedOnLoadAverage(float threshold,
	                                                          double predictedValue, int min) {
	...
	public int getMaxNumberOfInstancesRequired(int numberOfInstancesRequiredBasedOnRif,
	                                           ...
	                                           boolean mcReset,
	                                           int numberOfInstancesReuquiredBasedOnLoadAverage,
	                                           boolean laReset) {
	...
	public PartitionAlgorithm getPartitionAlgorithm(String partitionAlgorithm) {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("Retrieving partition algorithm [Partition algorithm]: %s",
			                        partitionAlgorithm));
		}
		if (StratosConstants.PARTITION_ROUND_ROBIN_ALGORITHM_ID.equals(partitionAlgorithm)) {
			autoscaleAlgorithm = new RoundRobin();
		} else if (StratosConstants.PARTITION_ONE_AFTER_ANOTHER_ALGORITHM_ID
				.equals(partitionAlgorithm)) {
			autoscaleAlgorithm = new OneAfterAnother();
		} else {
			if (log.isErrorEnabled()) {
				log.error(String.format("Partition algorithm %s could not be identified !",
				                        partitionAlgorithm));
			}
		}
	 ...
	 */
	public void delegateSpawn(ClusterLevelPartitionContext clusterMonitorPartitionContext,
	                          String clusterId, String clusterInstanceId, boolean isPrimary,
		...
		try {
			...
			MemberContext memberContext = AutoscalerCloudControllerClient.getInstance()
			                                                             .startInstance(
			...
			if (memberContext != null) {
				...
				partitionContext
						.addMemberStatsContext(new MemberStatsContext(memberContext.getMemberId()));
				if (log.isDebugEnabled()) {
			...
			String message =
					String.format("Could not start instance: [cluster-id] %s [instance-id] %s",
	...
	public void delegateScalingDependencyNotification(String clusterId, String networkPartitionId,
	                                                  String instanceId, int requiredInstanceCount,
	                                                  int minimumInstanceCount) {

		if (log.isDebugEnabled()) {
			log.debug("Scaling dependent notification is going to the [parentInstance] " +
			          instanceId);
		}
	...
	public void delegateScalingOverMaxNotification(String clusterId, String networkPartitionId,
	                                               String instanceId) {
		if (log.isDebugEnabled()) {
			log.debug(
					"Scaling max out notification is going to the [parentInstance] " + instanceId);
		}
		//Notify parent for checking scaling dependencies
		ClusterMonitor clusterMonitor =
				AutoscalerContext.getInstance().getClusterMonitor(clusterId);
	...
	public void delegateScalingDownBeyondMinNotification(String clusterId,
	                                                     ...
	                                                     String instanceId) {
		if (log.isDebugEnabled()) {
			log.debug("Scaling down lower min notification is going to the [parentInstance] " +
			          instanceId);
		}
		//Notify parent for checking scaling dependencies
		ClusterMonitor clusterMonitor =
	...
	public void delegateTerminate(ClusterLevelPartitionContext clusterMonitorPartitionContext,
	                              String memberId) {
		...
		try {
			...
			if (clusterMonitorPartitionContext.activeMemberAvailable(memberId)) {
				...
				log.info(String.format(
						"[scale-down] Moving active member to termination pending list [member id] %s " +
				...
				clusterMonitorPartitionContext
						.moveActiveMemberToTerminationPendingMembers(memberId);
	...
	public void delegateTerminateDependency(
			ClusterLevelPartitionContext clusterMonitorPartitionContext, String memberId) {
		try {
	...
	public int getPredictedReplicasForStat(int minReplicas, float statUpperLimit,
	...
	public double getLoadAveragePredictedValue(ClusterInstanceContext clusterInstanceContext) {
		...
		for (ClusterLevelPartitionContext partitionContext : clusterInstanceContext
				.getPartitionCtxts()) {
			for (MemberStatsContext memberStatsContext : partitionContext.getMemberStatsContexts()
			                                                             .values()) {
	...
	public double getMemoryConsumptionPredictedValue(
			ClusterInstanceContext clusterInstanceContext) {
