...
public class AutoscalerCloudControllerClient {
	...
	private AutoscalerCloudControllerClient() {
		try {
			...
			stub._getServiceClient().getOptions()
			    .setProperty(HTTPConstants.SO_TIMEOUT, cloudControllerClientTimeout);
			stub._getServiceClient().getOptions()
	...
	public synchronized MemberContext startInstance(PartitionRef partition, String clusterId,
		...
		try {
			...
			instanceContext.setPartition(
					AutoscalerObjectConverter.convertPartitionToCCPartition(partition));
			...
			if (log.isDebugEnabled()) {
				...
				log.debug(String.format("Service call startInstance() returned in %dms",
				                        (endTime - startTime)));
			}
	...
	public synchronized void createApplicationClusters(String appId,
		...
		if (applicationClusterContexts != null) {
			for (ApplicationClusterContext applicationClusterContext : applicationClusterContexts) {
				if (applicationClusterContext != null) {
					...
					dto.setProperties(AutoscalerUtil.toStubProperties(
							applicationClusterContext.getProperties()));
					dto.setDependencyClusterIds(
		...
		org.apache.stratos.cloud.controller.stub.domain.ApplicationClusterContext[]
				...
				new org.apache.stratos.cloud.controller.stub.domain.ApplicationClusterContext[contextDTOs
						.size()];
	...
	public void terminateInstance(String memberId) throws Exception {
		if (log.isInfoEnabled()) {
			log.info(String.format("Terminating instance via cloud controller: [member] %s",
			                       memberId));
		}
		long startTime = System.currentTimeMillis();
		...
		if (log.isDebugEnabled()) {
			long endTime = System.currentTimeMillis();
			log.debug(String.format("Service call terminateInstance() returned in %dms",
			                        (endTime - startTime)));
		}
	}
	...
	public void terminateInstanceForcefully(String memberId) throws Exception {
		if (log.isDebugEnabled()) {
			log.debug(String.format(
					"Terminating instance forcefully via cloud controller: [member] %s", memberId));
		}
	...
	public void terminateAllInstances(String clusterId)
		...
		if (log.isInfoEnabled()) {
			log.info(String.format(
					"Terminating all instances of cluster via cloud controller: [cluster] %s",
					clusterId));
		}
		long startTime = System.currentTimeMillis();
		...
		if (log.isDebugEnabled()) {
			long endTime = System.currentTimeMillis();
			log.debug(String.format("Service call terminateInstances() returned in %dms",
			                        (endTime - startTime)));
		}
	}
}
