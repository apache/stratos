 ...
 */
public class VMClusterContext implements Serializable {
    ...
    private float requiredReplicas = 0;
    ...
    public VMClusterContext(String kubernetesClusterId, String clusterId) {
    ...
    private class PendingMemberWatcher implements Runnable {
        private VMClusterContext ctxt;

        public PendingMemberWatcher(VMClusterContext ctxt) {
            this.ctxt = ctxt;
        }
    ...
    private class ObsoletedMemberWatcher implements Runnable {
        ...
        public ObsoletedMemberWatcher(VMClusterContext ctxt) {
            this.ctxt = ctxt;
        }
        ...
        public void run() {
            while (true) {
                ...
                while (iterator.hasNext()) {
                    Entry<String, MemberContext> pairs = iterator.next();
